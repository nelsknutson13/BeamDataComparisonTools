# -*- coding: utf-8 -*-
"""
Created on Tue Feb 10 11:21:31 2026
...
def select_file():
    ...
    # show a friendly summary in the entry (first file + count)
...
def run_comparison():
    global SELECTED_FILES, COMBINED_PDD, COMBINED_PROFILES

    COMBINED_PDD = []
    COMBINED_PROFILES = []

    # Backward compatible: if user pasted a single path, accept it
...
def process_one_file(fn):
    """Your original single-file logic, lifted intact and called per file."""
        ...
        with open(fn, 'r', errors='ignore') as f:
            for line in f:
                ...
                if total_lines and (processed_lines == 1 or processed_lines == total_lines or processed_lines % 200 == 0):
                    ...
                    status_label.config(text=f"Processing {os.path.basename(fn)}... {progress_percent:.1f}%")
                ...
                if 'CURVETYPE' in s:
                    if 'TPR' in s:
                        ...
                        axis = 'X'
                        scan_type = 'CROSSPLANE'
                        scan_counts['CROSSPLANE'] += 1
                    elif 'INPLANE' in s:
                        axis = 'Y'
                        scan_type = 'INPLANE'
                        scan_counts['INPLANE'] += 1
                    axes_seen.add(axis)
...
# ---------------- main window ----------------
root = tk.Tk()
root.title("PTW data conversion tool V2")
# ---- Make GUI resizable ----
root.grid_rowconfigure(4, weight=1)   # row with summary_text
